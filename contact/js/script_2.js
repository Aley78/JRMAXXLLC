(function ($) {
  ...
  const wdtTabsWidgetHandlerInit = function ($scope) {
    ...
    $(document).ready(function () {
      const $tabs = $('.wdt-tabs-list li');
      const $defaultActiveTab = $tabs.filter('.ui-tabs-active');

      // Set the default styling for the active tab and its preceding tabs
      const defaultActiveIndex = $tabs.index($defaultActiveTab);
      $tabs.each(function (index) {
        const $tabContent = $(this).find('.ui-tabs-anchor .wdt-template-content .wdt-content-title');
        if (index <= defaultActiveIndex) {
          $tabContent.addClass('highlight-before');
        } 
      });
    });
    ...
    $self.tabInit = function ($) {
      ...
      $scope.find('.wdt-tabs-container').tabs({
        ...
        activate: function (event, ui) {
          ...
          // Call function to adjust background color
          $self.adjustTabBackground(ui.newTab);
        }
      });
      $tabInstance = $scope.find('.wdt-tabs-container').tabs('instance');
    };
    ...
    $self.mobileToggleInit = function () {
      ...
      $self.adjustTabBackground = function ($clickedTab) {
        const $tabs = $scope.find('.wdt-tabs-list li');
        const clickedIndex = $tabs.index($clickedTab);

        // Apply class to the clicked tab and all preceding tabs
        $tabs.each(function (index) {
          if (index <= clickedIndex) {
            $(this).find('.ui-tabs-anchor .wdt-template-content .wdt-content-title').addClass('highlight-before');
          } else {
            $(this).find('.ui-tabs-anchor .wdt-template-content .wdt-content-title').removeClass('highlight-before'); // Remove class for tabs after the clicked tab
          }
        });
      };
