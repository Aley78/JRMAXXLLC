(function ($) {
  ...
  const wdtImageBoxWidgetHandlerInit = function($scope) {
    ...
    // let $radialParentEl = $('#wdt-module-id-' + $widgetId);
    // let $radialfirstSibEl = $radialParentEl.children();

    // $radialfirstSibEl.each(function() {
    //     $(this).on("mousemove", function(e) {
    //         let rect = this.getBoundingClientRect();
    //         let x = e.clientX - rect.left;
    //         let y = e.clientY - rect.top;

    //         $(this).css("background", `radial-gradient(circle at ${x}px ${y}px, rgba(0, 255, 255, 0.25), transparent 30%)`);
